...
- One finding per issue, which is the same rule pointed the other way and the one we break. Do not report one defect at two anchors. A audit of our output found one defect written up at three separate anchors with three different titles, and 11% of everything we posted was a restatement of another finding in the same review. If you have already written up a defect, do not write it again at its call site, at its definition, or with a different trigger for the same broken behaviour. Pick the anchor a reader would want to land on — usually where the fix goes — and write it once.
