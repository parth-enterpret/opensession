...
describe("handleEnvelope", () => {
	test("hello does not ack", async () => {
		const r = recorder();
		await handleEnvelope(JSON.stringify({ type: "hello", num_connections: 1 }), r.deps);
	...
	test("slash_commands acks and dispatches nothing", async () => {
		const r = recorder();
		await handleEnvelope(
			JSON.stringify({ type: "slash_commands", envelope_id: "env-s", payload: {} }),
			r.deps,
		);
...
describe("openConnection", () => {
	...
	test("sends the app token and returns the socket url", async () => {
		...
		globalThis.fetch = (async (_url: string, init: RequestInit) => {
			...
			return new Response(JSON.stringify({ ok: true, url: "wss://slack/link?ticket=1" }));
